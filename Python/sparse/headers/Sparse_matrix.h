...
//	void sortindices();                         // Sorting indices based on a particular axis
	...
	void cumsum();	     // To be changed as a single function that may take axis as arguement
...
void  Sparse_matrix :: sq_root()		// Square root of the matrix
{
//	Sparse_matrix a(num_dims,num_nzvals,nz_vals,shape,indices);
	for(int i = 0; i < num_nzvals; i++)
		nz_vals[i] = sqrt(nz_vals[i]);
//	return a;
}
...
Sparse_matrix Sparse_matrix :: slices(int **axes)
{
